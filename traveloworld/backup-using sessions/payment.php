...
<form action="#" method="post" id="form-payment">
  ...
  <input type="hidden" name="gid" id="gid" value="<?php echo $_SESSION['id'];  ?>">
...
<?php if ($paymentSuccess==true){ $paymentSuccess=false; ?>
        ...
        $.ajax({
    url: "https://traveloworld.azurewebsites.net/public/index.php/user/"+uID,
    method: 'GET',
    contentType: 'application/json',
    dataType: 'JSON',
    
    success: function (data) {
         var newID=data.user_Id;
         idnew= newID;
         console.log(idnew);
          $('#user_Id').val(idnew);
         $('#myForm').submit();   
        ...
        $('#btn-pay').click(function () {
            ...
            if(flightStautsUpdated==true && bookingRoom=='no'){
                //$("#modalSuccess").modal('show');
                window.location.href = "https://traveloworld.azurewebsites.net/payment.php?payment=success&fID="+fID+"&user_Id="+user_Id;
            }
            //if paying for flight and rooms
            else if(flightStautsUpdated==true && roomStautsUpdated==true){
                //$("#modalSuccess").modal('show');
                window.location.href = "http://traveloworld.azurewebsites.net/payment.php?payment=success&fID="+fID+"&user_Id="+user_Id+"&rID="+rID;
            }
        });
  });
