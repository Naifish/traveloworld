...
if(!isset($_SESSION) || empty($_SESSION['token'])){ header('location:google-login.php');}
else{ 
...
if (isset($_GET['st']) && $_GET['st']=='Completed' && isset($_GET['item_name']) ) {
  ...
  $con = $con->connect();
    ...
    $exeQry = $con->query(" INSERT INTO my_bookings_atlantic (uID,rID) VALUES('$uID','$rID')");
                    if ($exeQry==true){
                        $paymentSuccess=true;
...
<div class="row">
  ...
  <div class="form-group">
    <label >Hotel <small>Atlantic Hotel</small></label>
    <label>Room number: <?php echo $_GET['rID'];?></label>
  </div>
  ...
  <form action="<?php echo $paypalURL; ?>" method="post">
        ...
        <input type="hidden" name="item_name" value="<?php echo $_GET['rID']; ?>">
        <input type="hidden" name="currency_code" value="CAD">
        
        <!-- Specify URLs -->
        <input type='hidden' name='cancel_return' value='https://atlantic-hotel.azurewebsites.net/index.php'>
        <input type='hidden' name='return' value='https://atlantic-hotel.azurewebsites.net/payment.php'>
